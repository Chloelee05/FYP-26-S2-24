...
public class CreateAuctionServlet extends HttpServlet {
    ...
    private static class AuctionFormInput {
        ...
        String[] tagIds;
        float price;
        BigDecimal maxPriceParsed; // null when not provided
        ...
        if (input.maxPrice != null && !input.maxPrice.isBlank()) {
            try {
                ...
                if (input.price > 0 && input.maxPriceParsed.compareTo(BigDecimal.valueOf(input.price)) <= 0) {
                    errorHandler(req, resp, "Maximum price must be greater than the starting price", input);
                    return false;
                }
            } catch (NumberFormatException e) {
                errorHandler(req, resp, "Invalid maximum price", input);
                return false;
            }
        }
