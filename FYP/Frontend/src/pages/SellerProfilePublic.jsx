...
export default function SellerProfilePublic() {
  ...
  if (!seller) {
    return <div className="max-w-5xl mx-auto px-4 py-16 text-center text-ink-400">Loading seller profile…</div>;
  }
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="card p-8 mb-6">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-sm text-ink-400">({seller.reviewCount ?? 0} reviews)</span>
              </div>
          ...
          <button onClick={() => setShowReport(v => !v)} className="flex items-center gap-1 text-sm text-ink-400 hover:text-red-400">
            <Flag size={14} /> Report
          </button>
        </div>
        ...
        {showReport && (
          <form onSubmit={handleReport} className="mt-4 flex gap-2">
            <input
              ...
              required
              className="flex-1 border border-ink-200 rounded-lg px-3 py-2 text-sm focus:outline-none"
            />
      ...
      {seller.reviews?.length > 0 && (
        <>
          <h2 className="section-title text-base mb-4">Reviews</h2>
          <div className="space-y-3 mb-6">
            {seller.reviews.map((r, i) => (
              <div key={i} className="card p-4">
                <div className="flex items-center gap-2 mb-1">
                  <StarRating value={r.score ?? r.rating ?? 0} size={14} />
                  <span className="text-xs text-ink-400">{r.buyerUsername ?? r.reviewer ?? 'Buyer'}</span>
                </div>
                {r.comment && <p className="text-sm text-ink-600">{decodeHtmlEntities(r.comment)}</p>}
              </div>
            ))}
          </div>
        </>
